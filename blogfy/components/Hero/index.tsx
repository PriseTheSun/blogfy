    ...
    return (
        <section className="max-w-7xl mx-auto px-6 py-16">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <article className="relative rounded-2xl overflow-hidden group col-span-1 lg:col-span-6 h-140 bg-gray-200 transform-gpu transition-transform duration-300 hover:scale-105 hover:shadow-2xl">
                    {main?.thumbnail && (
                        <Image src={main.thumbnail} alt={main.title} fill className="object-cover" />
                    )}

                    <div className="absolute inset-0 bg-linear-to-t from-black/90 to-transparent p-8 flex flex-col justify-end transition-all duration-300 group-hover:from-black/95">
                        ...
                        <h2 className="text-3xl lg:text-4xl text-white font-semibold leading-tight">{main?.title}</h2>
                        <p className="text-white/90 mt-3 max-w-2xl">{main?.description}</p>
                ...
                <div className="col-span-1 lg:col-span-6 flex flex-col gap-6 h-140">
                    {topRight && (
                        <article className="relative rounded-xl overflow-hidden group h-70 bg-gray-200 transform-gpu transition-transform duration-300 hover:scale-105 hover:shadow-lg">
                            {topRight.thumbnail && <Image src={topRight.thumbnail} alt={topRight.title} fill className="object-cover" />}
                            <div className="absolute inset-0 bg-linear-to-t from-black/80 to-transparent p-6 flex flex-col justify-end transition-all duration-300 group-hover:from-black/95">
                                ...
                                <h3 className="text-white font-semibold text-lg leading-tight">{topRight.title}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-6 h-65">
                        {bottom.map((post) => (
                            <article key={post.slug} className="relative rounded-xl overflow-hidden group h-full bg-gray-200 transform-gpu transition-transform duration-300 hover:scale-105 hover:shadow-md">
                                {post.thumbnail && <Image src={post.thumbnail} alt={post.title} fill className="object-cover" />}
                                <div className="absolute inset-0 bg-black/70 p-3 flex flex-col justify-end transition-all duration-300 group-hover:bg-black/80">
                                    ...
                                    <h3 className="text-white font-semibold text-sm leading-tight">{post.title}</h3>
                                    <div className="text-white/80 text-xs mt-2 flex items-center justify-between">
                                        <div>{post.author}</div>
